...
def main():
    ...
    even_better_draw_circles(rg.Point(100,50), 8, 6, 'blue', 8)
...
def better_draw_circles(point, delta_r):
    """
    ...
    for k in range(1, 11):  # k becomes 1, 2, 3, ... 10
        ...
        turtle.pen_down()
        turtle.draw_circle(delta_r * k)  # Radius 15, 30, 45, 60, ...
...
def even_better_draw_circles(point, delta_r, num_of_circles, pen_color, pen_thickness):
    turtle = rg.SimpleTurtle()
    ...
    for k in range(1, num_of_circles):  # k becomes 1, 2, 3, ... 10
